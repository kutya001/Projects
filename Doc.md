...
## 5. Архитектурные особенности интерфейса

### 1. Табличное представление (`TableView.js`)
...
- **In-Header фильтры**: в заголовках колонок (`<th>`) встроены иконки воронки, открывающие поповер с поиском и чек-боксами уникальных значений.
- **Индикатор применённых фильтров**: красный бейдж `Применено фильтров: N ✕ Сбросить`, моментально очищающий срезы и строку поиска.

### 2. Карточка объекта (`ViewForm.js`)
- **Компоновка**: Номер/код расположен в самом верху (`ПРОЕКТЫ · 76.1.`), а название объекта — строго под ним.
- **Отсутствие визуального шума**: избыточное троекратное дублирование названия удалено. Верхняя плашка оформлена как компактный бар статусов и быстрых действий.
...
- Встроенная SQL-консоль с горячей клавишей `Ctrl+Enter` и готовыми шаблонами (`PRAGMA table_info`, `COUNT(*)`).

### 4. Журнал аудита (`logs.js`)
